...
const DEBUG_LOG = process.env.XSJ_REWIND_DEBUG || ''
function dbg(msg) { if (!DEBUG_LOG) return; try { fs.appendFileSync(DEBUG_LOG, new Date().toISOString() + ' ' + msg + String.fromCharCode(10)) } catch {} }
...
export function apply(ctx) {
  ...
  function ensurePatched(session, st) {
    ...
    Object.defineProperty(session, 'deriveMessages', {
      ...
      value: function deriveMessagesWithRewind() {
        ...
        const events = typeof this.snapshotEvents === 'function' ? this.snapshotEvents() : this.events
        const out = []
  ...
  function resolveSession(sid) {
    ...
    const direct = sessions.get(sid)
    if (direct !== undefined) return direct
    // 适配 rc.1: 宿主 id 带 "session-" 前缀而客户端可能发裸 uuid
    dbg('resolve sid=' + JSON.stringify(sid))
    const bare = sid.replace(/^session-/, '')
    try {
      let first = true
      for (const cand of sessions.list()) {
        if (first) { first = false
          dbg('cand shape: type=' + typeof cand + ' keys=' + JSON.stringify(Object.keys(cand || {}).slice(0, 14)))
        }
        const cid = String(cand?.id ?? cand ?? '')
        if (cid === sid || cid === 'session-' + bare || cid.replace(/^session-/, '') === bare) {
          dbg('resolved->' + cid.slice(0, 45))
          return cand
        }
      }
    } catch (e) { dbg('list ERR ' + e) }
    return undefined
  }
  ...
  async function onMark(req, res) {
    ...
    const target = typeof session.eventAt === 'function' ? session.eventAt(seq) : session.events?.[seq]
  ...
  const withErrLog = (fn) => async (req, res) => {
    try { await fn(req, res) }
      ...
      console.error('[xsj-rewind]', e?.stack || String(e)); dbg('HANDLER-ERR ' + (e?.stack || String(e)).slice(0, 400))
      try { writeJson(res, 500, { ok: false, error: String(e?.message || e).slice(0, 120) }) } catch {}
    }
  }
