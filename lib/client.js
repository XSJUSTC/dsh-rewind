...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CancelButton(props) {
      ...
      // Boundary between "existed at mark time" and "arrived after": the mark
      // event's own seq. Only a user/steering node anchored BEYOND it is the
      // committing send — later messages that already existed must stay hidden.
      var boundary = pending ? pending.markSeq : null
      ...
      // Commit detection: a user/steering node anchored past the mark boundary
      // means the rewound draft was sent and the host has sealed the range.
      React.useEffect(function () {
        if (boundary === null) return
        if (firstNewInputAnchor(nodes, order, boundary) === null) return
      ...
      React.useEffect(function () {
        ...
        var upper = boundary === null ? null : firstNewInputAnchor(nodes, order, boundary)
        var keys = []
        for (var i = 0; i < order.length; i++) {
          var key = order[i]
          var n = nodes.get(key)
